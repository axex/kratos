...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('backend.role.index') }}">用户管理 - 角色</a></li>
...
@section('content')
    ...
    <form method="post" action="{{ route('backend.role.store') }}" accept-charset="utf-8">
        {{ csrf_field() }}
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane active" id="tab_1">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="name" value="{{ old('name') }}" placeholder="角色(用户组)名">
                    </div>
                    <div class="form-group">
                        <label>角色(用户组)展示名 <small class="text-red">*</small> <span class="text-green small">展示名可以为中文</span></label>
                        <input type="text" class="form-control" name="display_name" value="{{ old('display_name') }}" placeholder="角色(用户组)展示名">
                    </div>
                    <div class="form-group">
                        <label>角色(用户组)描述 </label>
                        <textarea class="form-control" name="descrition" cols="45" rows="2" maxlength="200">{{ old('descrition') }}</textarea>
                    </div>
                    <div class="form-group">
                        <label>关联权限</label>
