    ...
    <div class="container">
      ...
      <div class="overview">
        ...
        <form action="/subscribe" method="post" class="subscribe">
          {{ csrf_field() }}
          <input class="email" value="{{ old('email') }}" placeholder="请输入email" name="email">
    ...
    <div class="issues container">
      ...
      <ol class="issue-list">
        @foreach($issues as $issue)
          <li><span class='date'>{{ $issue->created_at->toDateString() }}</span><a
                href='{{ route('issue', $issue->issue) }}'>Kratos第{{ $issue->issue }}期</a></li>
        @endforeach
      </ol>
    </div>
    <!-- 多说评论框 start -->
