...
class AdminIssueController extends Controller
{
    ...
    {
        return view('backend.issue.create');
    }

    /**
     * 发布日期没写则用当前时间
     *
     * @param Backend\IssueRequest $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(Backend\IssueRequest $request)
    {
        ...
        if ($status) {
            return redirect()->route('backend.issue.index')->with('message', trans('validation.notice.create_issue_success'));
        }
    ...
    public function edit($id)
    {
        ...
        return view('backend.issue.edit', compact('issue'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function update(Backend\IssueRequest $request, $id)
    {
        $issue = Issue::findOrFail($id);
        $publishedAt = $request->get('published_at');
        if (! $publishedAt) {
        ...
        if ($status) {
            return redirect()->route('backend.issue.index')->with('message', trans('validation.notice.update_issue_success'));
        }
        return back()->with('fail', trans('validation.notice.database_error'));
    }
    ...
    public function destroy($id)
    {
        ...
        $article->delete();
        return redirect()->route('backend.issue.index')->with('message', trans('validation.notice.delete_issue_success'));
    }
